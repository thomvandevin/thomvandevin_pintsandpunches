...
public class Arduino_serialprox_unity : MonoBehaviour
{
	public static int PortNum = 5331;
	public static string IPnum = "127.0.0.1";
	
	public static byte[] data = new byte[256];
	public static bool portopen = false; //if port is open or not
	
	public static Socket _SerialPort = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
	...
	{
		updateslower++;
		if(portopen && updateslower>=updateinterval){
			askforinput();
			updateslower = 0;
		}
	}// Update	
			...
			if (portopen)
				{
					...
					try{
						_SerialPort.Connect(IPAddress.Parse(IPnum), PortNum);
					}catch(SocketException e){
						print("Unable to connect to serverObj. Error: "+e.ToString());
					}
					portopen = true;
	...
	void askforinput() 
	{
		...
		if(SetInputs){
			...
			if (Errorhandling){
				//all outputs ON
				_SerialPort.Send(System.Text.Encoding.ASCII.GetBytes("o,1,1,1,1,1,1"));
			}else{
				//all outputs OFF
				_SerialPort.Send(System.Text.Encoding.ASCII.GetBytes("o,0,0,0,0,0,0"));
			}
			//disable SetInputs
			SetInputs = false;
		}else{
			_SerialPort.Send(System.Text.Encoding.ASCII.GetBytes("p"));
		}
		
		try{
			//Read the data coming from the arduino
			
			int recv = _SerialPort.Receive(data);
			
			incommingdata = System.Text.Encoding.ASCII.GetString(data);
			//if you think some data is missing, you MIGHT be right.. sometimes not all data gets converted correctly.
			//If so, open this for-loop and comment out the above string.
			/*for(int ai = 0; ai < data.Length; ai++){
				if(data[ai] > 31){
					incommingdata = incommingdata+System.Text.Encoding.ASCII.GetString(data,ai,1);
				}
			}*/
			if (Errorhandling){
				print("data = " + incommingdata);
			}
			//data is redy for use.
			UseData = true;
			
			//this is only here so that the Unity console doesn't nag about this var being unused.
			recv = 0;
			if(recv !=0){
				print("didn't reset recv");
			}
